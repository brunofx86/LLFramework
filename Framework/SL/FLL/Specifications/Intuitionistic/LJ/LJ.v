...
Inductive Connectives := OR | AND  | IMP .
(* no quantifiers *)
...
Definition rulesBC (c :connectives) :=
  ...
  | IMP => {| rb_rightBody := fun F G => MOr (atom (down F)) (atom (up G) );
                 rb_leftBody  := fun F G =>MAnd (Bang (atom (up F)) ) (atom (down G)) |}
  end.
...
#[local] Instance SimpleOORUles : OORules :=
  ...
  Ltac solveP :=
   match goal with
| [H: IsPositiveAtom (AAnd _ _) |- _ ] => inversion H 
| [H: IsPositiveAtom (MAnd _ _) |- _ ] => inversion H 
| [H: IsPositiveAtom (AOr _ _) |- _ ] => inversion H 
| [H: IsPositiveAtom (MOr _ _) |- _ ] => inversion H 
| [H: IsPositiveAtom (Bang _) |- _ ] => inversion H 
| [H: IsPositiveAtom (Quest  _) |- _ ] => inversion H 
end.

  Ltac solveTF :=
   match goal with
| [H: IsPositiveAtom (makeLRuleBin _ _ _) |- _ ] => inversion H 
| [H: IsPositiveAtom (makeRRuleBin _ _ _) |- _ ] => inversion H 
end.

Definition SNEG F := ((atom (up F))) ⊗ (? (perp (up F))).
  


Lemma CutCoherenceOR: CutCoherenceBin cutR1 (rulesBC OR).
...
Lemma CutCoherenceAND: CutCoherenceBin cutR1 (rulesBC AND).
Proof with sauto; try solveLL.
  unfold CutCoherenceBin;intros.
  simpl...
  LLPerm  [( ⌈ F ⌉^) ⊕ ( ⌈ G ⌉^); ⌊ F ⌋^].
...
Lemma CutCoherenceIMP: CutCoherenceBin cutR1 (rulesBC IMP).
Proof with sauto; try solveLL.
  unfold CutCoherenceBin;intros.
  simpl...
  LLPerm  ((⌊ F ⌋^ ⊗ ( ⌈ G ⌉^)):: ([]++[⌊G ⌋^])).
...
Proof with sauto.
 ...
 + exists BTwoPA.
  ...
  + exists BOneP.
    ...
    - apply FocusingPlus in H4...
      ...
      LLExact H2.
      lia. intros.
      simpl; solveLL. apply InvPlus... solveLL... LLExact H.
      exists [⌈ Go1 ⌉], x0, 4.
      split... do 2 constructor... 
      inversion H0...  inversion H... 
      right... 
      ...
      LLExact H2.
      lia. intros.
       simpl; solveLL. apply InvPlusComm... solveLL... LLExact H.
  + exists BOneP.
     do 3 intro. intros. 
     apply FocusingClause in H...
    - apply FocusingPlus in H4...
 ...
 + exists BTwoPM.
    ...
    TFocus (makeLRuleBin IMP Fo1 Go1).
      solveTF. 
      FLLsplit [⌊ t_bin IMP Fo1 Go1 ⌋ ] (Delta0). 
...
Qed.
 ...
 Lemma wellFormedBinaryI_p : wellFormedBinaryIL.
Proof with sauto.
  unfold wellFormedBinaryIL;intros.
  destruct lab.
 + exists BTwoPA.
     do 3 intro. intros. 
     apply FocusingClause in H...
    - apply FocusingWith in H4... 
      ...
      exists (atom (up F)). exists x, [ Fo1 ],  [ Go1 ], x1, 4.
      split...  constructor... 
      inversion H0...  inversion H3... 
      constructor... 
      inversion H0...  inversion H3... 
      left. split... rewrite H... simpl. solveLL. 
      apply seqNtoSeq in H2... LLExact H2. rewrite <- H1...
      apply seqNtoSeq in H5... LLExact H5. rewrite <- H1...
      LLExact H2.  rewrite <- H1... rewrite LEncodeApp...
      LLExact H5.  rewrite <- H1... rewrite LEncodeApp...
      lia. intros.
      TFocus (makeLRuleBin OR Fo1 Go1).
      solveTF. 
      FLLsplit [⌊ t_bin OR Fo1 Go1 ⌋ ]  (⌈ F ⌉  :: ⌞ Delta12 ⌟).
      simpl. solveLL. LLExact H4. LLExact H6.
    - apply FocusingWith in H4...
      exists (atom (up F)), Delta, [ Fo1 ],  [ Go1], x0, 4.
      split... constructor... 
      inversion H0...  inversion H... 
      constructor... 
      inversion H0...  inversion H... 
  ...
  + exists BOneP.
    ...
    - apply FocusingPlus in H4...
     ...
     left. exists x. split... 
      ...
      FLLleft. solveLL. 
      apply seqNtoSeq in H2...
      LLExact H2.  LLExact H2. rewrite <- H1 ... rewrite LEncodeApp...
      lia. intros.
      TFocus (makeLRuleBin AND Fo1 Go1).
      solveTF. 
      FLLsplit [⌊ t_bin AND Fo1 Go1 ⌋ ] (⌈ F ⌉ :: ⌞ Delta1 ⌟).
      FLLleft. solveLL. LLExact H4. rewrite LEncodeApp...
     ...
     left. exists x. split... 
      ...
      solveTF. 
      FLLsplit [⌊ t_bin AND Fo1 Go1 ⌋ ] (⌈ F ⌉ :: ⌞ Delta1 ⌟).
      FLLright. solveLL. LLExact H4. rewrite LEncodeApp...
     - apply FocusingPlus in H4...
       exists  F, [Fo1 ], x0, 4.
      split...
     constructor... 
...
exists  F, [Go1 ], x0, 4.
 ...
 + exists BTwoPM.
    ...
    - apply FocusingTensorPos in H4...
       apply checkEncodeCasesD in H3...  
       exists  F, Fo1. exists x, []. exists [Go1], x1, 4.
      left. split... rewrite H... 
      simpl. 
      FLLsplit (@nil oo)  (⌞ x ⌟ ++ [⌈ F ⌉]) ; solveLL. 
      apply seqNtoSeq in H2...
      apply seqNtoSeq in H5...
      LLExact H5. LLExact H5. rewrite LEncodeApp... rewrite <- H1...
      lia. intros. 
      LLPerm ([⌊ t_bin IMP Fo1 Go1 ⌋] ++ (⌈ F ⌉ ::  ⌞ Delta1 ⌟ ++ ⌞ Delta2 ⌟)).
     eapply InvTensorT'. exact H3. solveLL.  simpl.  solveLL. 
      LLPerm ((! ⌈ Fo1 ⌉) ⊗ ⌊ Go1 ⌋ :: ⌞ Delta2 ⌟ ++  ⌈ F ⌉ :: ⌞ Delta1 ⌟).
       eapply InvTensor';solveLL. LLExact H6. rewrite LEncodeApp...
    - apply FocusingTensorPos in H4...
       exists  F, Fo1. exists Delta, []. exists [Go1], x0, 4.
      right. split...  
      simpl. 
      FLLsplit (@nil oo)  (⌞Delta⌟ ++ [⌈ F ⌉]) ; solveLL. 
      apply seqNtoSeq in H2...
      apply seqNtoSeq in H5...
      LLExact H5.  rewrite LEncodeApp... 
      lia. intros. simpl;solveLL.
      LLPerm ((! ⌈ Fo1 ⌉) ⊗ ⌊ Go1 ⌋ :: ⌞ Delta2 ⌟ ++  ⌈ F ⌉ :: ⌞ Delta1 ⌟).
       eapply InvTensor';solveLL. LLExact H1. 
Qed.
...
(* conjunction *)
| LJAndL1 : forall L C F G, LJSeq P (F :: L) C -> LJSeq P ( (t_bin AND F G) :: L) C
| LJAndL2 : forall L C F G, LJSeq P (G :: L) C -> LJSeq P ( (t_bin AND F G) :: L) C
| LJAndR : forall L F G , LJSeq P L F -> LJSeq P L G -> LJSeq P L (t_bin AND F G)

(* implication *)
| LJImpL : forall L1 L2 C F G , LJSeq P L1 F -> LJSeq P (G::L2) C -> LJSeq P (t_bin IMP F G ::L1++L2) C
| LJImpR : forall L F G, LJSeq P (F::L) G ->  LJSeq P L (t_bin IMP F G )
...
Theorem SoundenessFLL: forall n L C, 
                  ...
                  isOLFormula C ->
                 LJSeq wC (n:=n) L C ->
                                seq (OLTheoryCut PnN n) (LEncode L)  (REncode [C]) (UP []).
  ...
  eapply H with (m:=x2)...
   ...
   - apply OLInPermutationL in H7... 
    ...
    ++ apply FocusingTensorPos in H8... 
         ...
         assert (Permutation ( (t_bin IMP F0 G :: t_bin IMP F0 G :: L1) ++ x0 ++ x0)
                     ((t_bin IMP F0 G :: x0) ++ (t_bin IMP F0 G :: L1++x0)))...
        rewrite H2.
          eapply LJImpL.
assert(LJSeq nC ([]++(t_bin IMP F0 G :: x0)) F0).
         eapply H with (m:=x1)...
          inversion H4...
         inversion H5...
         LLExact H6.
         ...
         rewrite H7...
       simpl in H5...
      assert(Permutation (G :: t_bin IMP F0 G :: L1 ++ x0) ((G::L1)++(t_bin IMP F0 G :: x0)))...
      rewrite H5. 
        eapply H with (m:=x1)...
          inversion H4...
         inversion H8...
