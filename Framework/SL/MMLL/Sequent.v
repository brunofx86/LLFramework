...
Section LLSequent.
  ...
  Inductive MLLN:  nat -> list TypedFormula -> list oo -> Arrow -> Prop :=
...
| tri_bangD : forall n B i,  tri_bangK4 n B i [] [] (UP [ ]) -> md i = true -> S n |-F- B ; [] ; UP []     
     with
      tri_bangK4 : nat -> list TypedFormula -> subexp -> list TypedFormula -> list oo -> Arrow -> Prop :=
       ...
       | tri_copyLK : forall b F n L M C B B' i, lt i b  -> m4 b = false -> u b = false ->
        Permutation ((b,F)::B') B -> tri_bangK4 n B' i C L (UP (M++[F])) -> 
                                          tri_bangK4 (S n) B i C L (UP M)                                    
       | tri_exp : forall B C i L M n, SetU B -> (* NoLtX i B -> *)
  ...
  Inductive MLLS:  list TypedFormula -> list oo -> Arrow -> Prop :=
...
| tri_bangD' : forall B i,  tri_bangK4' B i [] [] (UP []) -> md i = true -> |-f- B ; [] ; UP []
     with
      tri_bangK4' : list TypedFormula -> subexp -> list TypedFormula -> list oo -> Arrow -> Prop :=
       ...
       | tri_copyUK' : forall b F L M C B B' i, lt i b  -> m4 b = false -> u b = true ->
        ...
        Permutation ((b,F)::B') B -> tri_bangK4' B' i C L (UP (M++[F])) -> 
                                      tri_bangK4' B i C L (UP M)                                      
       | tri_exp' : forall B C i L M , SetU B -> (* NoLtX i B -> *)
