...
Require Export LL.Framework.SL.MMLL.Specifications.CutCoherence.
...
Inductive buildTheory  : oo ->  Prop :=
  ...
  | bQconnR : forall C FX, isOLFormula  (t_qcon C FX)  -> buildTheory  (makeRRuleQ C FX)
  | bQconnL : forall C FX, isOLFormula  (t_qcon C FX)  -> buildTheory  (makeLRuleQ C FX) .
...
Inductive OLTheory  P : oo -> Prop :=
  ...
  | ooth_strpos {f : pos P}: forall OO a, isOLFormula OO -> OLTheory P (POS a OO) 
  | ooth_strneg {f : neg P}: forall OO b, isOLFormula OO -> OLTheory P (NEG b OO) .
