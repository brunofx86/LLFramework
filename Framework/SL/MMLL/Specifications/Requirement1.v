...
Definition GenericBiPoleFail
  (Rule:  oo) : Prop :=
  forall n, MLLN theory n Gamma Delta ( DW Rule) -> False .
...
Definition GenericBiPoleAxiom
  ...
  forall n,
  MLLN theory n Gamma Delta ( DW Rule) ->
  isOLFormula connective ->
    ( (* case the atom is taken from the linear context *)
      (exists D1,
       Permutation Delta ( atom (predicate connective ) :: D1) /\
       MLLS theory  Gamma D1 (DW RBody) /\
       forall Delta1 Gamma1 (theory' : oo -> Prop),
       (theory'  (Rule)) ->
       MLLS theory' Gamma1  ( atom (predicate ((connective)) ) :: Delta1) (UP []))
     \/
    ( (* case the atom is taken from the classical context *)
       exists a, mt a = true /\ In (a,atom ( predicate connective )) Gamma /\
       MLLN theory 0  Gamma Delta (UP [RBody] ) /\  
       forall a Delta1 Gamma1 (theory' : oo -> Prop),
       (theory'  Rule) -> mt a = true ->
       In (a, atom (predicate (connective) )) Gamma1 ->
       MLLS theory' Gamma1 Delta1 (UP []))).
  ...
  posAtomFormulaL D1' /\ 
    (
      ( exists D1,
        Permutation Delta ((atom (predicate connective))::D1) /\ 
        MLLS theory  Gamma D1 ( DW RBody) /\
        MLLN theory n' Gamma (D1 ++ D1') ( UP [])  /\
        n'' > 0 /\
        n = plus n' n'' /\
        forall Delta1 Gamma1 (theory' : oo -> Prop),
        (theory'  (Rule)) ->
        MLLS theory' Gamma1 (Delta1 ++ D1') (UP []) -> 
         MLLS theory' Gamma1 ( (atom (predicate ((connective)) )) :: Delta1) (UP []))
     \/
      ( exists a, In (a,atom (predicate (connective))) Gamma /\ mt a = true /\
        MLLS theory  Gamma Delta (DW RBody) /\
        MLLN theory n' Gamma (Delta ++ D1') ( UP [])  /\
        n'' > 0 /\
        n = plus n'  n'' /\
        (forall Delta1 Gamma1 (theory' : oo -> Prop),
          MLLS theory' Gamma1 (Delta1 ++ D1') (UP []) ->
          MLLS theory' Gamma1 Delta1 (DW RBody))) ).
...
Definition GenericBiPole2PM
  ...
  ( 
    ( Permutation Delta (atom ((predicate connective )) :: (D1 ++ D2)) /\
       MLLS theory  Gamma (D1 ++ D2) (DW RBody) /\
       MLLN theory n' Gamma (D1 ++ D1') (UP []) /\
       MLLN theory n' Gamma (D2 ++ D2') (UP []) /\
       n'' > 0 /\
       n = plus n' n'' /\
       forall Delta1 Delta2 Gamma1 (theory' : oo -> Prop),
       theory' Rule ->
       MLLS theory' Gamma1 (Delta1 ++ D1') (UP []) ->
       MLLS theory' Gamma1 (Delta2 ++ D2') (UP []) ->
       MLLS theory' Gamma1 (atom ((predicate (connective))) :: Delta1 ++ Delta2) (UP []))
   \/
       (exists a, In (a,atom (predicate (connective) )) Gamma  /\ mt a = true /\ 
          ...
          MLLS theory' Gamma1 (Delta1 ++ D1') (UP []) ->
          MLLS theory' Gamma1 (Delta2 ++ D2') (UP []) ->
          MLLS theory' Gamma1 (Delta1 ++ Delta2) (DW RBody))).
...
Definition GenericBiPole2PA
  ...
  ( 
     ( Permutation Delta (atom ((predicate connective )) :: D12) /\
       MLLS theory  Gamma D12 (DW RBody) /\
       MLLN theory n' Gamma (D12 ++ D1') (UP []) /\
       MLLN theory n' Gamma (D12 ++ D2') (UP []) /\
       n'' > 0 /\
       n = plus n' n'' /\
       forall Delta12 Gamma1 (theory' : oo -> Prop),
       theory' Rule ->
       MLLS theory' Gamma1 (Delta12 ++ D1') (UP []) ->
       MLLS theory' Gamma1 (Delta12 ++ D2') (UP []) ->
       MLLS theory' Gamma1 ( atom ((predicate ( connective) )) :: Delta12) (UP [])
     )
   \/
     ( exists a, In (a,atom (predicate (connective))) Gamma /\ mt a = true /\
       MLLS theory  Gamma Delta (DW RBody) /\
       MLLN theory n' Gamma (Delta ++ D1') (UP []) /\
       MLLN theory n' Gamma (Delta ++ D2') (UP []) /\
       n'' > 0 /\
       n = plus n' n'' /\
       forall Delta12 Gamma1 (theory' : oo -> Prop),
       MLLS theory' Gamma1 (Delta12 ++ D1') (UP []) ->
       MLLS theory' Gamma1 (Delta12 ++ D2') (UP []) ->
       MLLS theory' Gamma1 Delta12 (DW RBody))).
